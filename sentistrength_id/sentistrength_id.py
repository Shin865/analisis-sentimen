...
for x in myresult:
##  print(x[1])
    hasil_s = senti.main(x[2])
    label_s = hasil_s['kelas']

    id_dataset = x[0]
    id_ds = str(id_dataset)
    string_hasil = str(hasil_s)

##query ke tabel dataset
    ##sql = "UPDATE dataset SET label_sentistrength = '"+label_s+"'WHERE id_dataset = '"+id_ds+"'"
    ##mycursor.execute(sql)
    ##mydb.commit()

    sql = "UPDATE dataset SET hasil_sentistrength = %s, label_sentistrength = %s WHERE id_dataset = %s"
    val = (string_hasil, label_s, id_dataset)
